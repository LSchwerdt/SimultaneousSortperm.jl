...
function uintmap_string(s::String, ::Type{T}, firstcodeunit::Int) where T<:Unsigned
    len = ncodeunits(s)
    lastcodeunint = min(len, sizeof(T) + firstcodeunit - 2)
    j = sizeof(T)
    x = zero(T)
    for i in firstcodeunit:lastcodeunint
        j -= 1
        x *= 258
        @inbounds x += T(codeunit(s, i)) + 1
    end
    while j > 1
        j -= 1
        x *= 258
...
uintmap_string(s::String, ::Type{T}) where T = uintmap_string(s, T, 1)[1]
